...
class Queue(models.Model):
    ...
    users = models.ManyToManyField(User, verbose_name=_('users with access'))
...
class TicketManager(models.Manager):
    def get_query_set(self):
        current_user = get_current_user()
        if current_user.is_superuser or current_user.is_anonymous():
            return super(TicketManager, self).get_query_set()
        else:
            return super(TicketManager, self).get_query_set().filter(queue__users__in=[current_user])
